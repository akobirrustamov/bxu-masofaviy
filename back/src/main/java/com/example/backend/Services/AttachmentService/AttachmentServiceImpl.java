    ...
    public class AttachmentServiceImpl implements AttachmentService {
        ...
        @Override
        public ResponseEntity<?> deleteFile(UUID id) {
            Optional<Attachment> attachmentOptional = attachmentRepo.findById(id);
            if (attachmentOptional.isPresent()) {
                Attachment attachment = attachmentOptional.get();
                ...
                if (file.exists() && file.delete()) {
                    ...
                    return ResponseEntity.status(500).body("Faylni o‘chirishda xatolik yuz berdi");
                }
            }
            return ResponseEntity.status(404).body("Fayl topilmadi");
        }
    }
